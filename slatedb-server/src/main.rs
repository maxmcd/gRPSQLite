...
pub struct MemoryVfs {
    db: Arc<Mutex<Db>>,
    leases: Arc<Mutex<HashMap<String, String>>>, // lease_id -> database_name
...
impl MemoryVfs {
    async fn new() -> Result<Self, SlateDBError> {
        ...
        let kv_store = Db::open("test_db", object_store).await?;
        Ok(Self {
    ...
    async fn validate_lease(&self, lease_id: &str, expected_database: &str) -> Result<(), Status> {
        ...
        match leases.get(lease_id) {
            Some(database) => {
                ...
                // Check if expected_database is a journal file for this database
                if expected_database.ends_with("-journal") {
                    let base_name = expected_database.strip_suffix("-journal").unwrap();
                    if database == base_name {
                        return Ok(());
                    }
                }

                println!(
                    "Lease {lease_id} is for database '{database}', not '{expected_database}'"
                );
            ...
            None => {
                ...
                Err(Status::not_found(format!("Lease {lease_id} not found")))
            }
        }
    }
}
...
impl grpsqlite::grpsqlite_server::Grpsqlite for MemoryVfs {
    async fn get_capabilities(
        ...
        println!(
            "get_capabilities readonly={} client_token={}",
            inner.readonly, inner.client_token
        );
        return Ok(Response::new(grpsqlite::GetCapabilitiesResponse {
            context: self.next_id().to_string(),
            atomic_batch: true,
    ...
    async fn acquire_lease(
        ...
        println!(
            "acquire_lease ctx={} database={}",
            inner.context, inner.database
        );

        let mut leases = self.leases.lock().await;

        // Check if database already has a lease
        for (existing_lease_id, existing_database) in leases.iter() {
            if existing_database == &inner.database {
                return Err(Status::already_exists(format!(
                    "Database '{}' already has an active lease: {}",
                    inner.database, existing_lease_id
                )));
            }
        }

        let lease_id = self.next_id().to_string();

        // Store the lease mapping
        leases.insert(lease_id.clone(), inner.database.clone());

        println!(
            "Acquired lease {} for database {}",
            lease_id, inner.database
        );

        Ok(Response::new(grpsqlite::AcquireLeaseResponse { lease_id }))
    }
    ...
    async fn close(
        ...
        let inner = request.into_inner();
        println!(
            "close context={} lease_id={} file={}",
            inner.context, inner.lease_id, inner.file_name
        );

        if inner.file_name.ends_with("-journal") {
            println!("close journal file {}", inner.file_name);
            return Ok(Response::new(()));
        }

        // Remove the lease if it exists
        let mut leases = self.leases.lock().await;
        if let Some(database) = leases.remove(&inner.lease_id) {
            println!("Released lease {} for database {database}", inner.lease_id);
        }
    ...
    async fn read(
        ...
        // Validate lease
        self.validate_lease(&inner.lease_id, &inner.file_name)
            .await?;
